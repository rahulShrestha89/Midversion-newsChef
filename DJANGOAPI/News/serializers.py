...
class NewSerializer(serializers.HyperlinkedModelSerializer):
	owner = serializers.ReadOnlyField(source='owner.username')
	highlight = serializers.HyperlinkedIdentityField(view_name='new-highlight', format='html')

	class Meta:
		model = New
		fields = ('url','pk','highlight','owner','title','code','lineos','language','style')


class UserSerializer(serializers.HyperlinkedModelSerializer):
	News = serializers.HyperlinkedRelatedField(many=True,view_name='new-detail', read_only=True)

	class Meta:
		model = User
		fields = ('url','pk','username','News')
